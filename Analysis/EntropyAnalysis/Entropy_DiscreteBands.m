...
for i = 1:length(input_files)
    ...
    % Save
    EEG_delta = pop_saveset(EEG_delta, 'filename', [curr_file(1:end-4) '_delta.set'], 'filepath', output_folder);

    % Pass band filter theta (4-8 HZ)
    EEG_theta = pop_eegfiltnew(EEG, 4, 8, [], 0);

    % Save
    EEG_theta = pop_saveset(EEG_theta, 'filename', [curr_file(1:end-4) '_theta.set'], 'filepath', output_folder);

    % Pass band filter alpa (8-12 Hz)
    EEG_alpha = pop_eegfiltnew(EEG, 8, 12, [], 0);
    
    % Save
    EEG_alpha = pop_saveset(EEG_alpha, 'filename', [curr_file(1:end-4) '_alpha.set'], 'filepath', output_folder);

    % Pass band filter beta (13-30 Hz)
    EEG_beta = pop_eegfiltnew(EEG, 13, 30, [], 0);

    % Save
    EEG_beta = pop_saveset(EEG_beta, 'filename', [curr_file(1:end-4) '_beta.set'], 'filepath', output_folder);
end
...
file_extension = '*.set';
output_folder = '/mnt/raid/RU1/Raw_data/Ettore/Entropy/Healthy/Entropy_Analysis_FreqBands/'
mkdir(output_folder)
eeglab_folder = '/mnt/raid/software/eeglab2024.1/';
addpath(genpath(eeglab_folder));
eeglab;
...
% Set folders
work_dir = '/mnt/raid/RU1/Raw_data/Ettore/Entropy/Healthy/Entropy_Analysis_FreqBands/';
cd(work_dir);
